...
const word = (input: Uint32Array, index: number): number => input[index] ?? 0;

const sha256Hex = (input: string): string => {
  const inputBytes = new TextEncoder().encode(input);
  const bitLength = inputBytes.length * 8;

  const totalLength = Math.ceil((inputBytes.length + 1 + 8) / 64) * 64;
  ...
  for (let offset = 0; offset < totalLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      const w15 = word(w, index - 15);
      const w2 = word(w, index - 2);
      const w16 = word(w, index - 16);
      const w7 = word(w, index - 7);
    ...
    for (let index = 0; index < 64; index += 1) {
      const k = HASH_K[index] ?? 0;
      const wi = word(w, index);
      const s1 = rightRotate(e, 6) ^ rightRotate(e, 11) ^ rightRotate(e, 25);
      const ch = (e & f) ^ (~e & g);
      const temp1 = (((((h + s1) >>> 0) + ((ch + k) >>> 0)) >>> 0) + wi) >>> 0;
